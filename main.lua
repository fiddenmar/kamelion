...
function collideHeroWithTile(dt, shape_a, shape_b, mtv_x, mtv_y)
	...
	if shape_a == hero and shape_b.type ~= hero.type then
		hero_shape = shape_a
		tileshape = shape_b
	elseif shape_b == hero and shape_a.type ~= hero.type then
		hero_shape = shape_b
		tileshape = shape_a
	else
	...
	if hero.kamelion then
		if tileshape ~= "gray" then
			return
		end
	end

	if hero.adopt and (math.abs(mtv_x) > 8 or math.abs(mtv_y) > 8) then
		hero.adopt = false
		hero.type = tileshape.type
		return
	end
...
function handleInput(dt)
	...
	if love.keyboard.isDown("left") then
		hero:move(-hero.xSpeedLimit*dt, 0)
	end
	if love.keyboard.isDown("right") then
		hero:move(hero.xSpeedLimit*dt, 0)		
	end
	if love.keyboard.isDown("up") then
		hero:move(0, -(hero.xSpeedLimit+50)*dt)
	end
...
function updateHero(dt)
	...
	hero.ySpeed = hero.ySpeed - world.gravity * dt
	hero:move(0, - hero.ySpeed)
	if hero.type == "dark_gray" then
		setupHero(startX, startY)
	end
end
...
function findTiles(map)
	local colourTiles = {}
	local ground_types = {"red", "green", "blue", "gray", "dark_gray", "finish", "start"} 
